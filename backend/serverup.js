    ...
    if (containerName) {
      ...
      try {
        if (fs.existsSync(containerFolder)) {
          ...
          folderRemovalResult = { success: true, message: `Server folder ${containerFolder} removed successfully` };
          console.log(`Server folder removed: ${containerFolder}`);
        } else {
          folderRemovalResult = { success: false, message: `Server folder ${containerFolder} not found` };
        }
      } catch (folderError) {
        folderRemovalResult = { 
          success: false, 
        ...
        console.error('Failed to remove server folder:', folderError);
      }
    }
...
// Read file content
app.get("/files/content", authenticateToken, (req, res) => {
  const requestedPath = req.query.path;
  const containerName = req.query.container;
...
// Save file content
app.post("/files/save", authenticateToken, (req, res) => {
...
// Upload file
app.post("/files/upload", authenticateToken, upload.single('file'), (req, res) => {
  if (!req.file) {
...
// Extract ZIP file
app.post("/files/extract", authenticateToken, (req, res) => {
...
app.post("/files/mkdir", authenticateToken, (req, res) => {
...
// Get process logs
app.get("/processes/:id/logs", authenticateToken, (req, res) => {
  const { id } = req.params;
...
// Kill process
app.post("/processes/:id/kill", authenticateToken, (req, res) => {
  const { id } = req.params;
...
app.delete("/processes/:id", authenticateToken, (req, res) => {
  const { id } = req.params;
...
app.post("/console/execute", authenticateToken, (req, res) => {
...
app.post("/console/run", authenticateToken, (req, res) => {
...
app.get("/startup-commands", authenticateToken, (req, res) => {
  const commands = Array.from(startupCommands.values());
  res.json({ commands });
});

// Save startup command
app.post("/startup-commands", authenticateToken, (req, res) => {
...
app.delete("/startup-commands/:id", authenticateToken, (req, res) => {
  const { id } = req.params;
...
// Run startup command
app.post("/startup-commands/:id/run", authenticateToken, (req, res) => {
  const { id } = req.params;
...
app.post("/scripts/javascript", authenticateToken, (req, res) => {
...
app.post("/scripts/python", authenticateToken, (req, res) => {
  const { code, workingDir } = req.body;
  if (!code) {
...
app.post("/scripts/javascript/run", authenticateToken, (req, res) => {
...
app.post("/scripts/python/run", authenticateToken, (req, res) => {
  const { code, workingDir, name } = req.body;
  if (!code) {
    return res.status(400).json({ error: "Python code required" });
  }
...
app.get("/docker/images", authenticateToken, async (req, res) => {
  try {
...
app.post("/docker/execute", authenticateToken, (req, res) => {
...
// External API endpoints for programmatic access
